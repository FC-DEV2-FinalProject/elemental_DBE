...
@RestController
@RequestMapping("/tiger/board/faq")
...
public class FaqController {
    ...
    private String fileUploadPath = "/upload/bbs/faq";

    private final Long managerNum = 3L;

    // . 전체 조회 및 검색
    @GetMapping
    public BoardPageResponseDto searchFaq(@RequestParam("searchType") String type,
        ...
        Optional<Page<BoardSelectResponseDto>> optionalFaqPage = boardService.SearchBoardByKeyword(type, keyword, pageable, managerNum);
        Page<BoardSelectResponseDto> faqPage = optionalFaqPage.orElse(Page.empty());

        // faqPageResponseDto로 변환 후 반환
        return new BoardPageResponseDto(
                faqPage.getContent(), // 콘텐츠 리스트
                faqPage.getTotalElements(), // 전체 데이터 개수
                faqPage.getTotalPages(), // 전체 페이지 수
                faqPage.getNumber() // 현재 페이지 번호
        );
    }

    //  상세 조회
    @GetMapping("/{id}")
    public BoardResponseDto getFaqById(@PathVariable("id") Long boardNum) {
    ...
    public ResponseEntity<Void> createFaq(HttpSession session ,
    ...
    //  수정
    @PutMapping("/{id}")
    public ResponseEntity<Void> updateFaq(@PathVariable("id") Long boardNum ,
                                          HttpSession session,
                                          HttpServletRequest request ,
                                          @RequestBody BoardInsertRequestDto boardInsertRequestDto,
                                          @RequestPart(value = "miniMap", required = false) MultipartFile file) throws IOException {
